//THIS CODE INCLUDES MATT W. CHANGES -- BUT CHANGES BROKE THE CART
...
export default function CartProvider({ children }) {
  ...
  const addPrimaryBoothToCart = async (boothId) => {
    ...
    if (
      tier1.some((tier) => currentVendor.sponsorshipLevel.includes(tier))  //changed from sponsorship.leve to sponsorshipLeve
      //ERROR HERE:  "Unhandled Rejection (TypeError): Cannot read properties of null (reading 'includes')"
      //maybe prob b/c returns a true or false, not a value??? not handling "null" values??
      //In firebase, there are values of "null" for sponsorship.level
      //-- also, maybe incomplete with conditions in rest of function??
    ) {
  ...
  const addSecondaryBoothToCart = async (boothId) => {
    ...
    const booth = booths.find((b) => b.id === boothId)
    if (
      tier1.some(tier => currentVendor.sponsorshipLevel.includes(tier))  //changed from sponsortship.level to sponsorshipLevel
    ) {
      if (booth.hasElectricity) {
        const checkout = await addItemToCart(
          'doubleBooth',
...
//   //EXAMINE THIS FUNCTION -- SEEMS OK -- but maybe--but prob not-- have to call getShopifyCart() again here??
//   const changeQuantity = (itemId, currentQuantity) => {
//     console.log('changeQuantity called')
//     console.log('changeQuantity itemId', itemId)
//     console.log('changeQuantity currentQuantity', currentQuantity)
//     console.log("currentVendor.id from changeQuantity", currentVendor.id)
//     let newQuantity = currentQuantity - 1
//     console.log('newQuantity from changeQuantity', newQuantity)
//     client.checkout
//       .updateLineItems(currentVendor.cartId, {
//         id: itemId,
//         quantity: newQuantity,
...
//     if (!currentVendor) return
//     console.log('opening cart')
//     console.log('currentVendor from openCart', currentVendor)
//     console.log('currentVendor.cartId from openCart', currentVendor.cartId)

//     client.checkout
//       .fetch(currentVendor?.cartId)
//       .then((checkout) => window.open(checkout.webUrl)) //problem with this checkout.webUrl in certain cases where goes to opveteran.org instead of checkout page
//   }
